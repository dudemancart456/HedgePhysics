...
public class AtmosphericScatteringSun : MonoBehaviour
{
    ...
    void OnEnable()
    {
        if (instance)
        {
            Debug.LogErrorFormat("Not setting 'AtmosphericScatteringSun.instance' because '{0}' is already active!",
                instance.name);
            return;
        }
    ...
    void OnDisable()
    {
        ...
        if (instance != this)
        {
            Debug.LogErrorFormat(
                "Not UNsetting 'AtmosphericScatteringSun.instance' because it points to someone else '{0}'!",
                instance.name);
            return;
        }
