...
public class AtmosphericScattering : MonoBehaviour
{
    ...
    [Header("World Components")] public Gradient worldRayleighColorRamp = null;
    ...
    [Header("Height Components")] public Color heightRayleighColor = Color.white;
    ...
    [Header("Sky Dome")] public Vector3 skyDomeScale = new Vector3(1f, 0.1f, 1f);
    ...
    [Header("Scatter Occlusion")] public bool useOcclusion = false;
    ...
    [Header("Other")] public float worldScaleExponent = 1.0f;
    public bool forcePerPixel = false;
    public bool forcePostEffect = false;

    [Tooltip("Soft clouds need depth values. Ignore means externally controlled.")] public DepthTexture depthTexture =
        DepthTexture.Enable;
    ...
    void OnEnable()
    {
        ...
        if (instance && instance != this)
            Debug.LogErrorFormat(
                "Unexpected: AtmosphericScattering.instance already set (to: {0}). Still overriding with: {1}.",
                instance.name, name);
    ...
    void OnDisable()
    {
        ...
        if (instance != this)
        {
            if (instance)
                Debug.LogErrorFormat(
                    "Unexpected: AtmosphericScattering.instance set to: {0}, not to: {1}. Leaving alone.", instance.name,
                    name);
        }
        else
        {
    ...
    void OnWillRenderObject()
    {
        ...
        if ((SystemInfo.graphicsShaderLevel >= 40 || depthTexture == DepthTexture.Enable) &&
        ...
        if (useOcclusion)
        {
            ...
            m_occlusionCmdBeforeScreen.GetTemporaryRT(occlusionId, occWidth, occHeight, 0, FilterMode.Bilinear,
    ...
    void UpdateStaticUniforms()
    {
        ...
        Shader.SetGlobalVector("u_ShadowBiasSkyRayleighMie",
            useOcclusion ? new Vector4(occlusionBiasSkyRayleigh, occlusionBiasSkyMie, 0f, 0f) : Vector4.zero);
        ...
        Shader.SetGlobalFloat("u_WorldNearScatterPush",
            -Mathf.Pow(Mathf.Abs(worldNearScatterPush), worldScaleExponent) * Mathf.Sign(worldNearScatterPush));
        ...
        Shader.SetGlobalVector("u_RayleighInScatterPct",
            new Vector4(1f - worldRayleighIndirectScatter, worldRayleighIndirectScatter, 0f, 0f));

        Shader.SetGlobalFloat("u_HeightNormalDistanceRcp", 1f / heightNormalDistance);
        Shader.SetGlobalFloat("u_HeightNearScatterPush",
            -Mathf.Pow(Mathf.Abs(heightNearScatterPush), worldScaleExponent) * Mathf.Sign(heightNearScatterPush));
    ...
    void UpdateDynamicUniforms()
    {
        ...
        var depthTextureScaledTexelSize = new Vector4(scale / pixelRect.width, scale / pixelRect.height,
